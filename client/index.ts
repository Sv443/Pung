...
import { colors, isEmpty, pause, mapRange } from "svcorelib";
...
let sock: WebSocket;
let act: ActionHandler;
/** Undefined if no internal server is hosted, else set to a ChildProcess instance (all inter-process communication is done through websocket) */
let internalServer: ChildProcess;
...
export async function run() {
    try {
        ...
        const host = useDefaultServer
            ? defaultServer
            : clientSettings.serverHost;
        const port = useDefaultServer
            ? cfg.defaultServerPort
            : clientSettings.serverPort;

        if (internal_server) await spawnInternalServer();
        ...
        act.on("error", async (err) => {
            ...
            if (err?.code === "ECONNREFUSED") {
                ...
                console.log(
                    `To check the status of the default server, visit ${cfg.statusURL}`
                );
                console.log(
                    `If this issue persists, please delete the client settings file at '${settings.settingsPath}' to restore your settings to the defaults and try again\n`
                );

                return closeConn();
            } else {
                console.log(
                    `${col.red}Socket error in ActionHandler:${col.rst}`
                );
                console.log(
...
async function mainMenu() {
    if (sock.readyState === 3) return run();
...
async function displayLobby() {
    ...
    const truncUser = (username, maxSpace) => {
        ...
        let spaces = "";
        for (let i = 0; i < spacesAmt; i++) spaces += " ";
    ...
    const lines = [
        ...
        ``,
        `┌────────────────────┬──────────────────────────┐`,
        `│    ${col.blue}Pung - Lobby${col.rst}    │    Join Code: ${
            col.green
        }${formatLobbyID(persistentData.lobbyID)}${col.rst}    │`,
        `├────────────────────┴───┬──────────────────────┘`,
        `│ You: ${col.green}${truncUser(persistentData.username, 16)}${
            col.rst
        } ${persistentData.isLobbyAdmin ? `${col.cyan} ♦${col.rst}` : "  "} │ ${
            col.yellow
        }${truncUser("TODO", 21)}${col.rst} ${
            persistentData.isLobbyAdmin ? "  " : `${col.cyan} ♦${col.rst}`
        } │`,
        ``,
        ``,
        ...
        ``,
        `You:      ${col.green}${persistentData.username}${col.rst}${
            persistentData.isLobbyAdmin ? ` ${col.cyan}(Admin)${col.rst}` : ""
        }`,
    ...
    lines.push(
        `${col.blue}Choose what to do:${col.rst} ${
            isAdmin ? `${col.yellow}[E]${col.rst}dit lobby settings • ` : ""
        }E${col.red}[x]${col.rst}it lobby `
    );
    ...
    if (validKey) clearTimeout(timeout);
}
...
function clearConsole() {
    if (!cfg.debugEnabled) console.clear();
    else console.log("\n\n<clear>\n\n");
}
...
async function incomingAction(action) {
    ...
    dbg(
        "IncomingAction",
        `Received action of type '${type}' from server, data: ${JSON.stringify(
            action.data
        )}`,
        "client"
    );
    ...
    switch (type) {
        ...
        case "lobbyNotFound": {
            ...
            console.log(
                `\nCouldn't find lobby with ID ${col.yellow}${formatLobbyID(
                    data.lobbyID
                )}${col.rst}\n`
            );
